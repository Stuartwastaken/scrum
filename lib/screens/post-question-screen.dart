...
class PostQuestionScreenWidget extends StatefulWidget {
  ...
  final String uid;
  const PostQuestionScreenWidget({Key? key, required this.isCorrect, required this.uid})
      : super(key: key);
...
Tuple2<String, String> getAbovePlayerInfo(List<MapEntry<String, dynamic>> sortedEntries, String uid) {
  List<Map<String, dynamic>> listOfMaps = sortedEntries.map((entry) => {entry.key: entry.value}).toList();
  print(listOfMaps);
  int thisPlayer = listOfMaps.indexWhere((element) => element['nickname'] == uid[0]);

  return Tuple2("one", "two");
}
...
class _PostQuestionScreenWidgetState extends State<PostQuestionScreenWidget> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<Map<String, dynamic>>(
        future: getUsersAndScores('998765'),   //------ hardcoded lobbyID. Will need to be changed
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            ...
            Tuple2<String, String> abovePlayerInfo = getAbovePlayerInfo(sortedEntries, widget.uid);
            return buildPostQuestionScreen(sortedEntries);
          }
  ...
  @override
  Widget buildPostQuestionScreen(List<MapEntry<String, dynamic>> sortedEntries) {
    final scaffoldKey = GlobalKey<ScaffoldState>();
    return Scaffold(
      key: scaffoldKey,
